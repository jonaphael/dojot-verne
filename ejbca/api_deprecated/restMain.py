...
from utils.enumList import Reasons

from config.ejbcaConf import ejbcaServ, initConf
...
def verifyCert(cacn, certsn):
    try:
        cert = zeep.helpers.serialize_object(
            ejbcaServ().checkRevokationStatus(cacn, certsn))
    except zeep.exceptions.Fault as error:
        return formatResponse(400, 'soap message: ' + error.message)
    resp = {
        'reason': Reasons(cert['reason']).name,
...
def getCert(cacn, certsn):
    try:
        cert = zeep.helpers.serialize_object(
            ejbcaServ().getCertificate(certsn, cacn))
    except zeep.exceptions.Fault as error:
        return formatResponse(400, 'soap message: ' + error.message)
    if cert is None:
...
def revokeCert(cacn, certsn):
    reasonCode = Reasons['UNSPECIFIED'].value
    if len(request.args) > 0:
        if 'reason' in request.args:
            try:
                reasonCode = Reasons[request.args['reason']].value
            except KeyError:
                return formatResponse(400, 'invalid revokation reason ' + request.args['reason'])
    try:
        resp = zeep.helpers.serialize_object(
            ejbcaServ().revokeCert(cacn, certsn, reasonCode))
    except zeep.exceptions.Fault as error:
        return formatResponse(400, 'soap message: ' + error.message)
...
def findCerts(username):
    ...
    try:
        certs = zeep.helpers.serialize_object(
            ejbcaServ().findCerts(username, onlyValid))
    except zeep.exceptions.Fault as error:
        return formatResponse(400, 'soap message: ' + error.message)
    if len(certs) == 0:
        return formatResponse(404, 'no certificates found')
